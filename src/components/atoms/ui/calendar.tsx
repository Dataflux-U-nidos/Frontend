...
function Calendar({
  ...
  const defaultClassNames = {
    ...
    day_button: cn(
      "relative cursor-pointer flex size-9 items-center justify-center whitespace-nowrap rounded-lg p-0 text-foreground outline-offset-2",
      ...
      "focus-visible:z-10",
      "[&:not([data-selected])]:hover:bg-accent",
      "[&:not([data-selected])]:hover:text-foreground",
      "group-data-[selected]:bg-primary",
      "group-data-[selected]:text-primary-foreground",
      "group-data-[selected]:group-[.range-middle]:bg-primary/20",
      "group-data-[selected]:group-[.range-middle]:text-primary", // ✅ Texto claro para días en el rango medio
      ...
      "group-data-[outside]:group-data-[selected]:text-primary-foreground",
    ...
    today:
      "*:after:pointer-events-none *:after:absolute *:after:bottom-1 *:after:start-1/2 *:after:z-10 *:after:size-[4px] *:after:-translate-x-1/2 *:after:rounded-full *:after:bg-primary [&[data-selected]:not(.range-middle)>*]:after:bg-background [&[data-disabled]>*]:after:bg-foreground/30 *:after:transition-colors",
    outside:
  ...
  const defaultComponents = {
    Chevron: (props: any) => {
      if (props.orientation === "left") {
        return <ChevronLeft size={16} strokeWidth={2} {...props} />;
      }
      return <ChevronRight size={16} strokeWidth={2} {...props} />;
    },
  };
